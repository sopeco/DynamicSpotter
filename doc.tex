...
\begin{enumerate}
  ...
  \item Start the target application (System under Test) by executing the following command line script:
	...
	If you see a line ending with\\\texttt{\ldots - Web-Server started on port 8081}\\ the target application has been
	started successfully:\\
	\begin{figure}[h!]
			\centering
			\includegraphics[width=\textwidth]{figures/quickStart/SNAG-0002.png}
			\label{fig:unpackedArchive}
		\end{figure}
	
	\textbf{IMPORTANT:} Do not stop the application!
  \item Open a \emph{second} command line window and again go to the \texttt{DEMO\_HOME} directory
  \item Start \DS analysis by executing the following command line script:
  \newline
	\newline
	\textbf{Windows:} \texttt{startDSfromCommandLine.bat}
	\newline
	\textbf{Linux:} \texttt{sh startDSfromCommandLine.sh}
	\begin{figure}[h!]
			\centering
			\includegraphics[width=\textwidth]{figures/quickStart/SNAG-0005.png}
			\label{fig:unpackedArchive}
		\end{figure}
		
		If \DS has been started successfully you should see progress report in the command line:
		\begin{figure}[h!]
			\centering
			\includegraphics[width=\textwidth]{figures/quickStart/SNAG-0006.png}
			\label{fig:unpackedArchive}
		\end{figure}
	\item In the demo scenario \DS runs about 15 minutes to execute all required experiments and analyze the measurement
	data.
	\item When \DS has finished its analysis it generates a result report in the \texttt{DEMO\_HOME/results} directory.
	There \DS creates a folder with the latest timestamp as folder name. Go to that directory. 
	\item The \texttt{SpotterReport.txt} file contains a textual description of which problems have been detected or not.
	\item If you further go to the individual directories of the anti-patterns (e.g. OLB\_2006070899), there you find the
	raw measurement data in the \texttt{csv} folder and (if available) some graphs in the \texttt{result-details} folder
	illustrating the problem:
	\begin{figure}[h!]
			\centering
			\includegraphics[width=0.8\textwidth]{figures/quickStart/SNAG-0007.png}
			\label{fig:unpackedArchive}
		\end{figure}
...
\end{figure}
The new project should appear in the Project Navigator (cf. Figure~\ref{fig:projectStructure}). 
...
\begin{figure}[h!]
\centering
\subfigure[Enabling Expert View
\label{fig:contextMenu2}]{
	\includegraphics[width=0.28\textwidth]{figures/demo/0015-expertView.png}
}
\subfigure[Hierarchy Editor
\label{fig:hierarchyEditor}]{
	\includegraphics[width=0.68\textwidth]{figures/demo/0016-hierarchy.png}
}
