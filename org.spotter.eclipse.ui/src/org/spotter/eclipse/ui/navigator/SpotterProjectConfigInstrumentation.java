 ...
 * 
 */
public class SpotterProjectConfigInstrumentation extends AbstractProjectElement {
	...
	private static final String OPEN_ID = InstrumentationEditor.ID;
	 ...
	 *            the parent element
	 */
	public SpotterProjectConfigInstrumentation(ISpotterProjectElement parent) {
		super(IMAGE_PATH);
		this.parent = parent;
		addHandler(OpenHandler.OPEN_COMMAND_ID, new IOpenable() {
			@Override
			public void open() {
				SpotterProjectConfigInstrumentation.this.open();
			}
	...
	private void open() {
		...
		AbstractSpotterEditor.openInstance(new InstrumentationEditorInput(file), OPEN_ID);
	}
