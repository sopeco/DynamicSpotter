...
public class CenterOfGravityStrategy {
	/**
	...
	public static List<Hiccup> findHiccups(Dataset rtDataSet, final NumericPairList<Long, Double> outRTSeries,
		...
		int windowSize = GlobalConfiguration.getInstance().getPropertyAsInteger("centerOfGravity.weightCalculationWindow", 31);
		int bucketSize = GlobalConfiguration.getInstance().getPropertyAsInteger("centerOfGravity.bucketSize", 101);
		...
		for (int i = 0; i < outRTSeries.size(); i += bucketSize) {

			int windowStart = i;
			int windowEnd = Math.min(i + bucketSize, outRTSeries.size() - 1);
			double weightedSum = 0.0;
			double sumOfWeigths = 0.0;
			long minTimestamp = Long.MAX_VALUE;
			long maxTimestamp = Long.MIN_VALUE;
			for (int j = windowStart; j <= windowEnd; j++) {
				long ts = outRTSeries.getPairs().get(j).getKey();
				if (ts < minTimestamp) {
					minTimestamp = ts;
				}
				if (ts > maxTimestamp) {
				...
				if (weights[j] == null) {
					weights[j] = calculateWeight(outRTSeries, windowSize, j);
				}
			...
			long timestamp = outRTSeries.getPairs().get((windowStart + windowEnd) / 2).getKey();
